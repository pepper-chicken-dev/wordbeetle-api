  ...
  describe "#perform" do
    context "期限切れゲストユーザが存在する場合" do
      ...
      it "削除件数をログに出力する" do
        ...
        expect(Rails.logger).to receive(:info).with("CleanupExpiredGuestsJob: Deleted 2 expired guest user(s)")
        described_class.perform_now
      end
    end

    context "期限内のゲストユーザが存在する場合" do
    ...
    context "期限切れゲストユーザに関連データが存在する場合" do
